...
class WifiUser(models.Model):
    ...
    id_document = models.CharField(max_length=20, blank=True, default='')
    networks = models.ManyToManyField('WifiNetworkLocation', related_name='users_nm', help_text="Networks this user has access to")
    ...
    android_version = models.CharField(blank=True, null=True)  # DEPRECATED: kept for DB compatibility with migration 0008
    def is_authorized_for_network(self, network: 'WifiNetworkLocation') -> bool:
        """
        Check if user is authorized to access a specific network.
        
        Authorization depends on:
        - User must be assigned to this network
        - If network requires validator: user must have valid allow_access_expiration (not expired)
        
        Args:
            network: WifiNetworkLocation instance to check authorization for
            
        Returns:
            bool: True if authorized for this network, False otherwise
        """
        # Check if user is assigned to this network
        if not self.networks.filter(pk=network.pk).exists():
            return False
        
        # If network requires validator, check expiration
        if network.requires_validator:
    ...
    def sign_csr(self, csr_pem: str, network: 'WifiNetworkLocation') -> tuple:
        """
        Sign a Certificate Signing Request (CSR) for the user using the specified network's CA.
        ...
        - User is authorized for the specified network
        
        Args:
            csr_pem (str): The PEM-encoded CSR to be signed
            network (WifiNetworkLocation): The network for which to sign the certificate
            
        Returns:
            tuple: (certificate_pem, ca_certificate_pem)
            
        Raises:
            ValueError: If CSR is invalid, CN doesn't match user, user not authorized for network, or required fields missing
        """
        # Verify user is assigned to this network
        if not self.networks.filter(pk=network.pk).exists():
            raise ValueError(f"User is not assigned to network '{network.name}'.")
        
        if not network.certificates_CA:
            raise ValueError(f"CA certificate not configured for network '{network.name}'.")
        ...
        # Get the certificate's CA from the network
        ca = network.certificates_CA
    ...
    def save(self, send_email: bool = True, *args, **kwargs):
        ...
        # If it does, check if the name, id_document, email or networks changed: 
        ...
        # Check if any assigned network has send_emails_automatically enabled
    ...
    def revoke_certificate(self):
        ...
        if self.certificate:
            ...
            # Mark all assigned networks' SSIDs to update CRL
            for network in self.networks.all():
                mark_ssid_to_update_crl(network)
        else:
    ...
    class Meta:
        ...
        ordering = ['name']
        unique_together = (('user_uuid', 'name'),)  # Ensure UUIDs are unique 
...
class WifiNetworkLocation(models.Model):
    ...
    def create_ca_certificates(self):
        ...
        # Notify all users assigned to this network that they need new certificates
        for user in self.users_nm.all():
